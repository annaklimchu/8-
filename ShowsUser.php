...
<body>
    ...
    if($result = $mysqli->query('SELECT * FROM `user` ')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
        ...
        printf("<table><tr><th>ІД</th><th>Імя</th><th>Фамілія</th>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['idusr'], $row['Name'],$row['Sumname'] ); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
