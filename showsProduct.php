...
<head>
    ...
    <title>Таблиця Proudct</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <h1>Таблиця Proudct</h1>
    ...
    if($result = $mysqli->query('SELECT * FROM `product` ')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит

        printf("Список : <br><br>");   // <br> в html - розрив рядка
        printf("<table><tr><th>ІД</th><th>Імя</th><th>Поставщік</th>");   // <br> в html - розрив рядка
        ...
        while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву
            printf("<tr><td>%s</td><td>%s</td><td>%s</td></tr>", $row['idp'], $row['Name'],$row['desor'] ); //виводимо результат на сторінку
        };
        printf("</table>");
        /*Звільняємо пам'ять*/
        $result->close();
